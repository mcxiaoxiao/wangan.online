...
9. 假设一个元组在子查询 1 中出现 m 次，在子查询 2 中出现 n 次，其中 m>0,n>0, 则：
   ...
   - **UNION <mark>ALL**</mark> 是包的操作，相同元组出现 <mark>**m+n**</mark> 次。
   - **EXCEPT <mark>ALL**</mark> 也是包的操作，出现 <mark>**max(0, m-n)**</mark> 次。
...
14. <mark>**where子句中不能包含聚集函数（eg：count(*)、sum(score)、avg(score)等）。聚集函数要等到所有行都扫描完、分组后才能算出结果（WHERE 的执行时机是“行级过滤”——在分组前），二者生命周期不同。**</mark>having+聚合如：
```sql
...
**包含键信息**：视图最好包含基本表的主键或唯一键，以确保更新操作能唯一标识每一行。<img src="./image/5-3.png" width="700" />
