...
- 列约束：是 **表约束的特例**，对 **单一列** 的值进行约束，包括：**NOT NULL、UNIQUE、PRIMARY KEY、CHECK (条件)** 等。作用分别是：

  - NOT NULL：该列的值不能为空。
  - UNIQUE：该列的值在表中必须是唯一的。
  - PRIMARY KEY：该列的值是 **主键**，唯一标识表中的每个元组。
  - CHECK (条件)：该列的值必须满足指定的条件。
...
![alt text](image/trigger-1.png)
![alt text](image/trigger-2.png)
---
...
1. 授权的**数据对象**的 **范围越小**，授权子系统 **越灵活**。因为子系统只需要检查授权的数据对象是否符合要求，而不需要检查其他数据对象。
