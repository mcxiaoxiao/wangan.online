...
![alt text](image/complicated-query.png)
### 🎆 复杂查询
...
![alt text](image/not-exist-1.png)
![alt text](image/not-exist-2.png)
...
![alt text](image/create-view.png)
...
类似：![alt text](image/5-4.png)选A，where中不能包含聚合函数，having中可以。
...
18. ![alt text](image/subqueryandgroupbyhaving.png)
...
![alt text](image/notin.png)

20. ![alt text](image/existfunc.png)A

21. ‍已知employee表中具有默认约束df_email，删除该约束的语句为：alter table employee <mark>drop constraint</mark> df_email

22. ![alt text](image/foreignkey.png) 复合外键 —— 当关联的目标表主键是 “多列组合” 时，外键就需要用多列来匹配，而非单一列。