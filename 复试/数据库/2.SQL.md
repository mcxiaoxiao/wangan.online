...
- __DML__：(Modify修改)Insert、Delete、Update、Select。
- __DCL__：(Ctrl控制)Grant、Revoke。
...
13. 存在having子句的查询，必须是group by，可以包含where子句。
14. where子句中不能包含聚集函数（eg：count(*)、sum(score)、avg(score)等）。聚集函数要等到所有行都扫描完、分组后才能算出结果（WHERE 的执行时机是“行级过滤”——在分组前），二者生命周期不同。