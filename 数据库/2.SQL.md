...
- <mark>__DCL__(Ctrl控制)：Grant(含义：授权)、Revoke(含义：撤销授权)。</mark>
...
14. <mark>**where子句中不能包含聚集函数（eg：count(*)、sum(score)、avg(score)等）。聚集函数要等到所有行都扫描完、分组后才能算出结果（WHERE 的执行时机是“行级过滤”——在分组前），二者生命周期不同。**</mark>group by+having+聚合如：
```sql
...
16. 
<img src="./image/5-5.png" width="700" /> B 子查询1 Except All 子查询2 的个数为 $m - n$(虽然不太严谨，应该是：max(m - n , 0))可惜其它错的更严重

17. where通配符： % 表示任意多个字符，_ 表示任意一个字符。

18. <img src="./image/subqueryandgroupbyhaving.png" width="700" />

UNION 合并两个子查询结果时，必须满足：
两个子查询的列数量完全相同
对应列的数据类型兼容
列的顺序必须完全一致
...
> ANY	、> SOME	大于任意一个（只要大于最小的就成立）
> ALL	无等价简写	大于所有（必须大于最大的才成立）
...
20. 关系模型 ≠ 关系模式 = 表的结构定义
安全性约束（A） 不属于 CREATE TABLE 的功能，它是只能通过 **权限管理（GRANT/REVOKE）** 实现的
<img src="./image/security.png" width="700" />



21. 在数据库的安全性控制中，授权的数据对象的范围越小 ，授权子系统就越灵活 