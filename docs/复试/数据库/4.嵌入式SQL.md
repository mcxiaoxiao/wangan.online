...
4. ![alt text](image/whenever.png)
5. 并发执行的多个事务之间可能互相影响，但不需要程序员考虑，而是需要DBMS予以保证
6. ![alt text](image/9-1.png)
7. ![alt text](image/9-2.png)B（勘误：5改成4）
8. ![alt text](image/9-3.png)A
...
![alt text](image/odbcjbjdbcdxqrsyb.png)
...
游标、fetch=`嵌入式SQL`
一条条、statement=`JDBC`
![alt text](image/dynamic-1.png)D

4. <mark>FOR UPDATE不是SQL的UPDATE语句（而是表达：“我要用这个游标去改 / 删数据，给我加行级锁，允许我修改当前行。”）。使用FOR UPDATE子句表示既能通过游标更新表数据，也能删除表数据。</mark>
![alt text](image/for-update.png)

5. 并发执行的多个事务之间可能互相影响，但不需要程序员考虑，而是需要DBMS予以保证
![alt text](image/recov.png)

6. 除了查看“:”的问题还要看c语言本身的bug
![alt text](image/execsql.png)

7. ![alt text](image/datadic.png)

8. JDBC使用三个CLASS：java.sql.DriverManager类(管理不同DBMS提供的不同的驱动程序)、java.sql.Driver类(不同DBMS提供的驱动程序)和java.sql.Connection类（建立数据库连接）连接到数据库![alt text](image/jdbcclass.png)
...
11. ![alt text](image/selectasclass.png)“学生按班级选课”是一个功能，不要受其影响。其最本质的还是“学生”选修“课程”，学生是有班级属性的，“按班级选课”可以通过程序予以实现，而E-R图应反映的是选课的结果是怎样的。故此选项最正确