...
- 💎 特性：ACID
  - 原子性：一组操作作为一个整体不可分；（事务的原子性是指事务中的所有操作要么全部执行成功，要么全部执行失败，不能部分执行。）
  - **一致性**：事务的操作状态是正确的；
  - **隔离性**：并发的各个事务之间互相不影响；
...
- 设置状态捕获语句：**`exec sql whenever 捕获条件 动作;`**
  ...
  - **whenever 的作用范围** 是其后所有 SQL 语句，直到 **程序中** 出现 **条件相同** 的 whenever 语句为止，且 **后面的覆盖前面的**。⚠ **whenever 的使用可能造成无限循环**。该语句之后执行的 SQL 操作若发生错误，程序会继续向下执行，不中断。该语句之前执行的 SQL 操作不受此规则约束，发生错误时会按默认方式处理（通常是终止或跳转）。
...
1. `where current of 游标名` 的作用是允许**更新或删除**当前游标数据。
...
5. 并发执行的多个事务之间可能互相影响，但不需要程序员考虑，而是需要DBMS予以保证
6. <img src="./image/9-1.png" width="600">
7. <img src="./image/9-2.png" width="600">B（勘误：5改成4）
8. <img src="./image/9-3.png" width="600">A
...
- **预编译执行**：先通过 `prepare` 语句将字符串变量进行 **预编译**，然后通过 `execute` 语句执行这个预编译后的语句，其中可以用 `using` 命令传入变量值。
  ...
  exec sql execute del_customer using :del_customer_id; //这里实际使用的是:del_customer_id的值
  ```
...
<img src="./image/odbcjbjdbcdxqrsyb.png" width="600">

2. **动态构造** SQL 语句时不要忘了加上 **逻辑连接符**（AND、OR、NOT）。

3. 句柄、prepare=`ODBC` 
游标、fetch=`嵌入式SQL`
一条条、statement=`JDBC`
<img src="./image/dynamic-1.png" width="600">D

4. <mark>FOR UPDATE不是SQL的UPDATE语句（而是表达：“我要用这个游标去改 / 删数据，给我加行级锁，允许我修改当前行。”）。使用FOR UPDATE子句表示既能通过游标更新表数据，也能删除表数据。</mark>
<img src="./image/for-update.png" width="600">

5. 并发执行的多个事务之间可能互相影响，但不需要程序员考虑，而是需要DBMS予以保证
<img src="./image/recov.png" width="600">

6. 除了查看“:”的问题还要看c语言本身的bug
<img src="./image/execsql.png" width="600">

7. <img src="./image/datadic.png" width="600">

8. JDBC使用三个CLASS：java.sql.DriverManager类(管理不同DBMS提供的不同的驱动程序)、java.sql.Driver类(不同DBMS提供的驱动程序)和java.sql.Connection类（建立数据库连接）连接到数据库<img src="./image/jdbcclass.png" width="600">

9. JDBC执行语句的是Statement对象（java.sql.Statement类是对特定的数据库执行SQL语句；java.sql. PreparedStatement类是用于执行预编译的SQL语句；java.sql.CallableStatement类是用于执行对数据库内嵌过程的调用；）

10. 数据字典(Data dictionary)，又称为系统目录(System Catalogs)是系统维护的一些“表”或“视图”的集合。数据字典通常是数据库管理员（而非用户）通过SQL-DDL(CREATE, ALTER, DROP)建立和维护的。。

11. <img src="./image/selectasclass.png" width="600">“学生按班级选课”是一个功能，不要受其影响。其最本质的还是“学生”选修“课程”，学生是有班级属性的，“按班级选课”可以通过程序予以实现，而E-R图应反映的是选课的结果是怎样的。故此选项最正确